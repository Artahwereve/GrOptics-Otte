...
*/
#ifndef GACTELESCOPEFACTORY
#define GACTELESCOPEFACTORY
...
/*!  /brief ACStdOptics structure stores details of a standard
...
struct ACStdOptics {
  ...
  // copy constructor
  ACStdOptics(const ACStdOptics &sco);
...
/*! \brief  GSegSCTelescopeFactory concrete class for constructing 
     ...
     factory method
 */
class GACTelescopeFactory : public GTelescopeFactory {
 private:

  friend class GReadACStd;

  GReadACStd *readAC;  //!< AC base reader
  ...
  map<int,ACStdOptics*> mStdOptics;    //*< map of standard telescopes
  map<int,ACStdOptics*>::iterator itmStdOp;  //*< iterator for this map
  ...
  ACStdOptics *opt;  //*< working ACstdOptics structure for current telescope
  
  GACTelescope *ACTel;  //*< pointer to working telescope
  int iNumACTelMade;
   ...
   */
  void editWorkingTelescope(GACTelescope *ACTel1);

 public:

  /*!  GACTelescopeFactory constructs a SC telescope from
   ...
   */
  GACTelescopeFactory(GReadACStd &scReader,
  ...
  /*! \brief makeTelescope constructs a SC telescope based on 
             ...
             \return GACTelescope pointer to constructed telescope
  */

  ~GACTelescopeFactory();

  GACTelescope *makeTelescope(const int &id,
				 const int &std);

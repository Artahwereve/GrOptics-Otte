...
GACTelescope::GACTelescope() {
  
  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::GACTelescope() " << endl;
  }
  initialize();
  };
/********************** end of GACTelescope *****************/
...
void GACTelescope::buildTelescope()
{
  bool debug = false;
  // fix units
  fFMeters = fF;
  ...
  if (debug) {
    *oLog << "  -- GACTelescope::buildTelescope" << endl;
  }
  gGeoManager = 0;
  fManager = new AOpticsManager("manager","The optics manager of AC");
...
/*************************************************************************************/
void GACTelescope::makePrimarySecondaryDisks() {

  bool debug = false;

  if (debug) {
    *oLog << "  --  GACTelescope::makePrimarySecondaryDisks" << endl;
  }
...
void GACTelescope::addPrimaryF() {
  bool debug = false;

  if (debug) {
    *oLog << "  --  GACTelescope::addPrimaryF" << endl;
   }
  Int_t count = 0;
...
/*******************************************************************/
void GACTelescope::addPrimaryMirror(const char*name,
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addPrimaryMirror" << endl;
  }
...
void GACTelescope::addSecondaryJ() {

  bool debug = false;
  
  if (debug) {
    *oLog << "  --  GACTelescope::addSecondaryJ" << endl;
  }
  Int_t count = 0;
  Int_t count1 = 0;
...
void GACTelescope::addPrimaryObscuration() {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::AddPrimaryObscuration " << endl;
  }
...
/*******************************************************************/
void GACTelescope::addSecondaryObscuration() {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::AddSecondaryObscuration " << endl;
  }
...
/*******************************************************************/
void GACTelescope::addSecondaryObscurationSeg(const char*name,
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addSecondaryObscurationSeg" << endl;
  }
...
/*******************************************************************/
void GACTelescope::addSecondaryMirror(const char*name, SegmentedMirror *mirror) {
  gGeoManager = fManager;
  
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addSecondaryMirror" << endl;
  }
...
/*******************************************************************/
void GACTelescope::addPrimaryBaffle() {
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addPrimaryBaffle" << endl;
...
/*******************************************************************/
void GACTelescope::addSecondaryBaffle() {
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addSecondaryBaffle" << endl;
...
/*******************************************************************/
void GACTelescope::addEntranceWindow() {
  gGeoManager = fManager;
 
  bool debug = false;
  if (debug) {
    *oLog << "  --  GACTelescope::addEntranceWindow" << endl;
...
/*************************************************************************************/
void GACTelescope::addIdealFocalPlane()  {
  bool debug = false;

  if (debug) {
    *oLog << "  --  GACTelescope::addIdealFocalPlane" << endl;
...
void GACTelescope::addMAPMTFocalPlane()  {
  bool debug = false;

  if (debug) {
    *oLog << "  --  GACTelescope::addMAPMTFocalPlane" << endl;
  }
...
void GACTelescope::closeGeometry()  {
  gGeoManager = fManager;
  fManager->CloseGeometry();
};
/*************************************************************************************/

void GACTelescope::testFocalPlane() {
...
void GACTelescope::injectPhoton(const ROOT::Math::XYZVector &photonLocT,
  ...
  bool debug = false;
  if (debug) {
    *oLog << " -- GACTelescope::injectPhoton " << endl;
    *oLog << "       photonLocT  ";
...
/********************** end of injectPhoton *****************/
void GACTelescope::movePositionToTopOfTopVol() {

  gGeoManager = fManager;

  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::movePositionToTopOfTopVol " << endl;
...
//****************************************************
bool GACTelescope::getCameraPhotonLocation(ROOT::Math::XYZVector *photonLoc,
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::getCameraPhotonLocation " << endl;
  }
...
void GACTelescope::printTelescope() {

  bool debug = false;
  if (debug) {
    *oLog << " -- GACTelescope::printTelescope" << endl;
    *oLog << "      iPrtMode " << iPrtMode << endl;
  }

  if (iPrtMode > 0) {
    *oLog << "   in GACTelescope::printTelescope() " << endl;
    *oLog << "      fF " << fF << endl;
  ...
  if (debug) *oLog << "exiting GACTelescope::printTelescope" << endl;
};
/********************** end of printTelescope *****************/

void GACTelescope::drawTelescope(const int &option) {

  bool debug = false;
  gGeoManager = fManager;

  if (debug) {
    *oLog << "  -- GACTelescope::drawTelescope" << endl;
...
void GACTelescope::setPrintMode(ostream &oStr,const int prtMode) {
  bool debug = false;
  iPrtMode = prtMode;

  if (debug) {
    *oLog << " -- GACTelescope::setPrintMode " << iPrtMode << endl;
  } 
};
/********************** end of setPrintMode *****************/

void GACTelescope::setPhotonHistory(const string &rootFile,
...
void GACTelescope::makePhotonHistoryBranches() {
...
void GACTelescope::fillPhotonHistory() {
...
void GACTelescope::initializePhotonHistoryParms() {
...
void GACTelescope::writePhotonHistory() {

  bool debug = false;
  if (debug) {
    *oLog << "  -- in GACTelescope::writePhotonHistory " << endl;
  }
  if (hisF != 0) {
    hisF->cd();  
    hisT->Write();
...
void GACTelescope::setReflCoeffMap(map<int, TGraph *> *mGr) {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::setReflCoeffMap" << endl;
...
TGraph * GACTelescope::makeReflectivityGraph(const Int_t &irefl) {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::makeReflectivityGraph" << endl;
    *oLog << "    size of mGRefl " << mGRefl->size() << endl;
  }
  Int_t id = irefl;
  TGraph *tmpNew;
...
void GACTelescope::initialize() {
  bool debug = false;

  if (debug) {
    *oLog << "  -- GACTelescope::initialization " << endl;
  }
...
void GACTelescope::CloseGeometry() {
  ...
  bool debug = false;
  if (debug) {
    *oLog << "  -- GACTelescope::CloseGeometry" << endl;
  }
  gGeoManager = fManager;
  fManager->CloseGeometry();
}; 
...
void GACTelescope::testPerformance() {

  // unused - from Akira's NewSCT.C, may not work here as is.
  /*
  bool debug = true;
  if (debug) {
    *oLog << "  -- GACTelescope::testPerformance " << endl;
  }

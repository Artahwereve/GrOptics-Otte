...
void GSegSCTelescope::addPrimaryF() {
  ...
  // Call TetragonSegmentedMirror by iterating over all mirrors
...
void GSegSCTelescope::addPrimaryMirror(const char*name,
  ...
  Double_t kF = 148.5 * cm;        // focal length
  Double_t kMirrorR = kF * 2;      // the radius of curvature
  TGeoTranslation* transZ = new TGeoTranslation(0, 0, kMirrorR); // Create translation for the mirrors
  TGeoHMatrix* hmat = new TGeoHMatrix((*combi) * (*transZ)); // Combine translation and combi from BuildMirrorCombiTrans
    
  fManager->GetTopVolume()->AddNode(mir, 1, hmat); //Apply the translation and rotation matrix
...
// Adds the camera.
  ...
  // Create a box for the camera and apply FocalSurface to it
  ...
  // End create a box for the camera and apply FocalSurface to it

  mapmt->AddNode(mapmtCathode, 1, new TGeoTranslation(0, 0, 0)); //Translation is 0 as it is positioned at the origin

  Double_t kF = 148.5 * cm;        // focal length

  //Add the camera to the volume
...
void GSegSCTelescope::injectPhoton(const ROOT::Math::XYZVector &photonLocT,
  ...
  */
  ...
  // ray = new ARay(0, fphotWaveLgt, (x/4)*m, (y/4)*m, z*m, t, dx*0, dy*0, dz); //Used for shorter computation times

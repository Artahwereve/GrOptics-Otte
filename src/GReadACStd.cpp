...
GReadACStd::GReadACStd(const string &pilotfile,GACTelescopeFactory *ACFactory  ) {

  // initialize variables
  bool debug = false;
  if (debug) {
    *oLog << "  -- GReadACStd::GReadACStd" << endl;
  }
...
GReadACStd::GReadACStd(const string &pilotfile) {
  spilotfile = pilotfile;

  bool debug = false;
  if (debug) {
    *oLog << "  -- GReadACStd::GReadACStd(const string pilotfile) "
          << pilotfile << endl;
  }
...
void GReadACStd::
setACTelescopeFactory(GACTelescopeFactory *ACFactory) {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GReadACStd::setACTelescopeFactory" << endl;
  }

  if (ACFac != 0 ) {
    cerr << "GReadACStd::setACTelescopeFactory " << endl;
    cerr << "   ERROR: ACFac pointer to ACFactory previously set" << endl;
...
/****************** end of setACTelescopeFactory **********/
void GReadACStd::setupACFactory() {

  bool debug = false;

  if (debug) {
    *oLog << "  -- GReadACStd::setupACFactory" << endl;
    //*oLog << "       spilotfile " << spilotfile << endl;
  }

  if (ACFac == 0) {
    cerr << " GReadACStd doesn't have a ACFactory object" << endl;
  ...
  while (pi->get_line_vector(tokens) >=0) {
    ...
    ACFac->mStdOptics[iStdOptNum] =  new ACStdOptics();
    opt = ACFac->mStdOptics[iStdOptNum];

    // set pointer in ACStdOptics for reflection coefficients.
    // maps to coefficients are now in all ACStdOptics objects.
    //opt->mVReflWaveLgts = ACFac->mVReflWaveLgts;
  ...
  */
  ...
  if (debug) {
    *oLog << endl;
    *oLog << "       printing all AC standard optics objects" << endl;
    for (ACFac->itmStdOp = ACFac->mStdOptics.begin();
    	 ACFac->itmStdOp != ACFac->mStdOptics.end();
    	 ACFac->itmStdOp++) {
      (*(ACFac->itmStdOp)).second->printACStdOptics();
    }
  }

  if (debug) {
    *oLog << "  -- end of setupACFactory" << endl;
  }

};

/******************** end of setupACFactory ****************/
 void GReadACStd::readBasicRecord(const vector<string> &tokens1,
                                     const MirSeg &eMirPS,
                                     ACStdOptics *opt1) {
...
void GReadACStd::readChangeRecord(const vector<string> &tokens1,
                       const MirSeg &eMirPS,
                                     ACStdOptics *opt1) {
  mirrorSegmentDetails segTmp;
  vector<int> vListSeg;    
...
void GReadACStd::getPolyCoeffs() {

  bool debug = false;
  if (debug) {
    *oLog << "  -- GReadACStd::getPolyCoeffs " << endl;
  }

  ifstream inFile(spilotfile.c_str(),ios::in);
  if (! inFile) {
    cerr << "  -- GReadACStd::getPolyCoeffs " << endl;
...
void GReadACStd::getReflCoeff() {
  ...
  if (debug) {
    *oLog << "  -- GReadACStd::getReflCoeff() " << endl;
    *oLog << "       spilotfile " << spilotfile << endl;
  }

  ifstream inFile(spilotfile.c_str(),ios::in);
  if (! inFile) {
    cerr << "  -- GReadACStd::getReflCoeff " << endl;
    cerr << "    could not open file: " << spilotfile << endl;
    exit(0);
  }
  ...
  while( getline(inFile,pilotline,'\n')) {
    ...
    if ( (tokens1.size() > 0) &&
      ...
      itGr =  ACFac->mGRefl->find(index);

      if (itGr!=ACFac->mGRefl->end()) {
        cerr << "trying to load reflection curve " << index << endl;
        cerr << "but curve already in the map: check pilot files " << endl;
        exit(0);
      }

      (*(ACFac->mGRefl))[index] = new TGraph(number);
      TGraph *grTmp = (*(ACFac->mGRefl))[index];
